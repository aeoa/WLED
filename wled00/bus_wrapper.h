...
static uint8_t wledC3RmtMemoryBlocks = 1;

// Gives NeoPixelBus both C3 TX memory blocks when only one RMT output needs them.
// The larger refill window mitigates Wi-Fi interrupt latency without enabling the
// high-priority C3 RMT driver, which is disabled because it can crash on RISC-V.
// Background: https://github.com/espressif/idf-extra-components/issues/473
static esp_err_t wledC3ConfigureRmt(rmt_config_t* config) {
  config->mem_block_num = wledC3RmtMemoryBlocks;
  return rmt_config(config);
}

#define rmt_config(config) wledC3ConfigureRmt(config)
#endif
// AI: end
...
class PolyBus {
  ...
  static void begin(void* busPtr, uint8_t busType, uint8_t* pins, uint16_t clock_kHz /* only used by DotStar */) {
    // AI: below section was generated by an AI
    #ifdef CONFIG_IDF_TARGET_ESP32C3
    // Dynamic RMT channels are assigned before any bus is initialized. A single
    // output always owns channel 0, so it can safely consume the adjacent block.
    wledC3RmtMemoryBlocks = _rmtChannelsAssigned == 1 ? 2 : 1;
    #endif
    // AI: end
    switch (busType) {
      case I_NONE: break;
