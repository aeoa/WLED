...
bool deserializeConfig(JsonObject doc, bool fromFS) {
  ...
  CJSON(espNowFloodRestrictPeers, nw[F("espnow_restrict")]);
  // AI: end
  ...
  if (!lrem.isNull()) {
    ...
    if (lrem.is<JsonArray>()) {
      const size_t remoteCount = min(lrem.size(), size_t(10));
      for (size_t i = 0; i < remoteCount; i++) {
        std::array<char, 13> entry{};
        getStringFromJson(entry.data(), lrem[i], 13);
        entry[12] = '\0';
        if (entry[0] != '\0') linked_remotes.emplace_back(entry);
      }
...
void serializeConfig(JsonObject root) {
  ...
  // AI: below section was generated by an AI
  nw[F("espnow_restrict")] = espNowFloodRestrictPeers;
  // AI: end
