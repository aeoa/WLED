  ...
  AsyncCallbackJsonWebHandler* handler = new AsyncCallbackJsonWebHandler(FPSTR(_json), [](AsyncWebServerRequest *request) {
    ...
    if (!isConfig) {
      ...
      */
      ...
      // Report local radio admission failures so clients can retry the same idempotent command.
      bool espNowFloodAccepted = true;
      verboseResponse = deserializeState(root, CALL_MODE_DIRECT_CHANGE, 0, &espNowFloodAccepted);
      if (!espNowFloodAccepted) {
        releaseJSONBufferLock();
        serveJsonError(request, 503, ERR_ESPNOW);
        return;
      }
      // AI: end
    } else {
