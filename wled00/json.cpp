...
bool deserializeState(JsonObject root, byte callMode, byte presetId, bool* espNowFloodAccepted)
{
  bool stateResponse = root[F("v")] | false;
  bool presetDirect = !root[F("pd")].isNull();
  if (espNowFloodAccepted) *espNowFloodAccepted = true;

  #ifndef WLED_DISABLE_ESPNOW
  // AI: below section was generated by an AI
  // "espf" is a transient API flag used by Wi-Fi clients to ask this node to originate
  // an ESP-NOW command flood. It is never persisted or forwarded as JSON.
  const bool espNowFloodRequested = root[F("espf")].is<bool>() && root[F("espf")].as<bool>();
  const bool espNowFloodOriginated = espNowFloodRequested && sendESPNowFloodCommand(root);
  if (espNowFloodAccepted && espNowFloodRequested) *espNowFloodAccepted = espNowFloodOriginated;
  if (espNowFloodRequested) {
    // The flood's ordered local apply queue owns these fields, including preset completion barriers.
    // Invalid flood requests must not silently degrade into a direct command on the gateway.
    root.remove(F("ps"));
    root.remove("on");
    root.remove("bri");
  ...
  if (timein != UINT32_MAX) {
    const bool hasMilliseconds = root[F("timems")].is<unsigned int>() && root[F("timems")].as<unsigned int>() < 1000U;
    if (hasMilliseconds) setTimeFromAPI(timein, root[F("timems")].as<unsigned int>());
    else setTimeFromAPI(timein);
    if (presetsModifiedTime == 0) presetsModifiedTime = timein;
    #ifndef WLED_DISABLE_ESPNOW
    // A direct app time update immediately makes this controller an eligible wall-clock root.
    if (hasMilliseconds && root[F("espt")].is<bool>() && root[F("espt")].as<bool>()) useESPNowFloodLocalTimeSource();
    #endif
  }
  ...
  #ifndef WLED_DISABLE_ESPNOW
  // AI: below section was generated by an AI
  if (espNowFloodOriginated) applyESPNowFloodTimebase();
  // AI: end
  #endif
...
void serializeInfo(JsonObject root)
{
  ...
  #ifndef WLED_DISABLE_ESPNOW
  // AI: below section was generated by an AI
  root[F("espf")] = 6; // v6 reports gateway admission failures and mesh recovery diagnostics
  JsonObject espNowMesh = root.createNestedObject(F("espmesh"));
  serializeESPNowFloodInfo(espNowMesh);
  // AI: end
  #endif
