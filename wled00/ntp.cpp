...
// Sets millisecond-accurate wall time supplied by a directly connected client.
void setTimeFromAPI(uint32_t timein, uint16_t milliseconds) {
  if (timein == 0 || timein == UINT32_MAX || milliseconds >= 1000) return;

  constexpr int64_t CLIENT_TIME_DEADBAND_MS = 1;
  constexpr int64_t CLIENT_TIME_STEP_THRESHOLD_MS = 50;
  const Toki::Time previous = toki.getTime();
  const int64_t error = (int64_t(timein) - previous.sec) * 1000LL + int32_t(milliseconds) - previous.ms;
  const uint64_t absoluteError = error < 0 ? uint64_t(-error) : uint64_t(error);
  // Keep a close NTP fix in preference to client time, but replace propagated time.
  if (toki.getTimeSource() > TOKI_TS_MS && absoluteError < 60000ULL) return;

  if (toki.getTimeSource() <= TOKI_TS_UDP_NTP || absoluteError > CLIENT_TIME_STEP_THRESHOLD_MS) {
    const Toki::Time supplied = {timein, milliseconds};
    toki.setTime(supplied, TOKI_TS_MS);
  } else if (error < -CLIENT_TIME_DEADBAND_MS) {
    Toki::Time corrected = previous;
    toki.adjust(corrected, -1);
    toki.setTime(corrected, TOKI_TS_MS);
  } else if (error > CLIENT_TIME_DEADBAND_MS) {
    Toki::Time corrected = previous;
    toki.adjust(corrected, 1);
    toki.setTime(corrected, TOKI_TS_MS);
  }
  if (absoluteError >= 60000ULL) {
    updateLocalTime();
    calculateSunriseAndSunset();
  }
  if (presetsModifiedTime == 0) presetsModifiedTime = timein;
