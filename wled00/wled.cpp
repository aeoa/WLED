...
// Defers fallback ESP-NOW initialization until the AP channel has settled after STA shutdown.
namespace {
  constexpr uint32_t ESP_NOW_FALLBACK_SETTLE_MS = 500;
  uint32_t espNowFallbackInitAt = 0;
}
// AI: end
#endif
...
void WLED::initESPNow()
{
  ...
  espNowUseAPInterface = wifiMode != WIFI_MODE_STA && wifiMode != WIFI_MODE_APSTA;
  if (!espNowUseAPInterface) {
    // In AP+STA fallback mode, keep ESP-NOW on the disconnected STA interface so an attached
    // phone cannot disturb it. Both interfaces still use the AP's current radio channel.
    ...
    espNowOK = quickEspNow.begin(apChannel, WIFI_IF_AP, false);
  }
  #else
  espNowUseAPInterface = apActive;
  espNowOK = apActive
      ? quickEspNow.begin(apChannel, WIFI_IF_AP, false)
      : quickEspNow.begin(CURRENT_WIFI_CHANNEL, WIFI_IF_STA, false);
  #endif
...
void WLED::handleConnection()
{
  ...
  // AI: below section was generated by an AI
  // Complete the AP fallback transition from the main loop, after the Wi-Fi event task has
  // applied the AP channel and station disconnect. Initializing inside that transition can bind
  // ESP-NOW to a stale interface/channel even though the library reports success.
  if (espNowFallbackInitAt && int32_t(now - espNowFallbackInitAt) >= 0) {
    espNowFallbackInitAt = 0;
    if (holdESPNowFallback) {
      initESPNow();
    }
  }
  // AI: end
    ...
    if (!apActive && now - lastReconnectAttempt > 12000 && (!wasConnected || apBehavior == AP_BEHAVIOR_NO_CONN)) {
      if (!(apBehavior == AP_BEHAVIOR_TEMPORARY && now > WLED_AP_TIMEOUT)) {
        ...
        if (enableESPNow && useESPNowSync) {
          ...
          WiFi.scanDelete();
          WiFi.disconnect();
          espNowFallbackInitAt = millis() + ESP_NOW_FALLBACK_SETTLE_MS;
