...
// Starts ESP-NOW on the interface that owns the radio channel after a Wi-Fi mode change.
void WLED::initESPNow()
{
#ifndef WLED_DISABLE_ESPNOW
  if (!enableESPNow) return;

  if (statusESPNow == ESP_NOW_STATE_ON) {
    DEBUG_PRINTLN(F("ESP-NOW restarting."));
    quickEspNow.stop();
    deinitESPNowFlood();
  }
  ...
  if (apActive) {
    ...
    espNowOK = quickEspNow.begin(CURRENT_WIFI_CHANNEL, WIFI_IF_STA, false); // Follow WiFi channel without blocking the main loop for send callbacks
  }
  statusESPNow = espNowOK ? ESP_NOW_STATE_ON : ESP_NOW_STATE_ERROR;
  if (!espNowOK) deinitESPNowFlood();
#endif
}
// AI: end
...
void WLED::initConnection()
{
  ...
  if (WLED_WIFI_CONFIGURED) {
    // Recreate the station netif before applying its IP configuration. On ESP32-C3,
    // WiFi.config() aborts if it races the WIFI_MODE_NULL teardown above.
    WiFi.mode(apActive ? WIFI_AP_STA : WIFI_STA);
    if (multiWiFi[selectedWiFi].staticIP != (uint32_t)0x00000000 && multiWiFi[selectedWiFi].staticGW != (uint32_t)0x00000000) {
...
void WLED::handleConnection()
{
  ...
  #ifndef WLED_DISABLE_ESPNOW
  bool holdESPNowFallback = wifiConfigured && apActive && enableESPNow && useESPNowSync;
  #else
  const bool holdESPNowFallback = false;
  #endif
  ...
  if (!Network.isConnected()) {
    ...
    if (now - lastReconnectAttempt > ((stac) ? 300000 : 18000) && wifiConfigured && !holdESPNowFallback) {
    ...
    if (!apActive && now - lastReconnectAttempt > 12000 && (!wasConnected || apBehavior == AP_BEHAVIOR_NO_CONN)) {
      if (!(apBehavior == AP_BEHAVIOR_TEMPORARY && now > WLED_AP_TIMEOUT)) {
        ...
        #ifndef WLED_DISABLE_ESPNOW
        // Keep stored credentials, but stop channel scans once the offline mesh fallback is active.
        if (enableESPNow && useESPNowSync) {
          WiFi.disconnect();
          initESPNow();
          holdESPNowFallback = true;
        }
        #endif
      }
    }
    if (apActive && apBehavior == AP_BEHAVIOR_TEMPORARY && now > WLED_AP_TIMEOUT && stac == 0 && !holdESPNowFallback) { // disconnect AP after 5min if no clients connected
