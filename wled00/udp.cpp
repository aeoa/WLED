...
void espNowReceiveCB(uint8_t* address, uint8_t* data, uint8_t len, signed int rssi, bool broadcast) {
  ...
  snprintf_P(last_signal_src, sizeof(last_signal_src), PSTR("%02x%02x%02x%02x%02x%02x"), address[0], address[1], address[2], address[3], address[4], address[5]);
  // AI: end
  ...
  if (!broadcast || buffer->magic != 'W' || !useESPNowSync || WLED_CONNECTED) {
