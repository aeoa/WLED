...
// Enables or disables scoped preset brightness and starts with the current on-brightness as its baseline.
void setRestorePresetBri(bool enabled) {
  if (restorePresetBri == enabled) return;
  restorePresetBri = enabled;
  presetBriOverride = false;
  briBase = bri > 0 ? bri : briLast;
}

// Starts a brightness override or restores the baseline before a brightness-less preset is applied.
void preparePresetBrightness(JsonObject preset) {
  if (!restorePresetBri || !preset["win"].isNull()) return; // HTTP API presets have opaque brightness semantics

  if (!preset["bri"].isNull()) {
    presetBriOverride = true;
    return;
  }

  if (!presetBriOverride) return;
  briLast = briBase;
  if (bri > 0) bri = briBase; // preserve the current off state
  presetBriOverride = false;
}
// AI: end
...
void handlePresets()
{
  ...
  if (!presetErrFlag) preparePresetBrightness(fdo);
  ...
  stateUpdated(tmpMode, true);  // was colorUpdated() if anything breaks
  #ifndef WLED_DISABLE_ESPNOW
  // AI: below section was generated by an AI
  applyESPNowFloodTimebase(tmpPreset);
  // AI: end
  #endif
