  ...
  switch (event) {
    case ARDUINO_EVENT_WIFI_AP_STADISCONNECTED:
      ...
      if (--apClients == 0 && isWiFiConfigured()) {
        ...
        // An offline ESP-NOW mesh owns the AP channel. Do not tear it down merely because the
        // phone left the gateway AP; Wi-Fi reconnects resume after ESP-NOW sync is disabled.
        if (!(enableESPNow && useESPNowSync)) forceReconnect = true;
        // AI: end
        #else
        forceReconnect = true;
        #endif
